...
func AddProductToCart(ctx context.Context, prodCollection, userCollection *mongo.Collection, productID primitive.ObjectID, user string) error {
	...
	user_id, err := primitive.ObjectIDFromHex(user)
	if err != nil {
		log.Println(err)
...
func RemoveCartItem(ctx context.Context, prodCollection, userCollection *mongo.Collection, productID primitive.ObjectID, user string) error {
...
func BuyItemFromCart(ctx context.Context, userCollection *mongo.Collection, user string) error {
	user_id, err := primitive.ObjectIDFromHex(user)
	if err != nil {
		log.Println(err)
		return ErrUserIdIsNotValid
...
func InstantBuyer(ctx context.Context, prodCollection, userCollection *mongo.Collection, productID primitive.ObjectID, User string) error {
	user_id, err := primitive.ObjectIDFromHex(User)
	if err != nil {
		log.Println(err)
		return ErrUserIdIsNotValid
